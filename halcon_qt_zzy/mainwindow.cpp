    ...
    , ui(new Ui::MainWindow)
{
    ...
    server = new QTcpServer(this);  //创建服务对象

    QList<QHostAddress> address = QNetworkInterface::allAddresses();  //获取tcpip
    for(QHostAddress tmp : address){
        if(tmp.protocol() == QAbstractSocket::IPv4Protocol){
            ui->comboBox_tcpip->addItem(tmp.toString());
        }
    }
    //服务端
    connect(server,&QTcpServer::newConnection,this,[=](){  //当tcp连接成功
        ui->pushButton_tcpout->setEnabled(true);
        ui->pushButton_tcpsend->setEnabled(true);
        ui->pushButton_newnumber->setEnabled(true);
        QTcpSocket* m_tcp = server->nextPendingConnection();
        ui->comboBox_tcpwhoid->clear();

        QList<QTcpSocket*> tcpSocketClients = server->findChildren<QTcpSocket*>();  //获取客户端号
        for(QTcpSocket* tmp:tcpSocketClients){
            if(tmp!=nullptr){
                ui->comboBox_tcpwhoid->addItem(QString::number(tmp->peerPort()));
                ui->textEdit_tcpacceptmsg->append("客户端："+QString::number(tmp->peerPort())+"已连接！");
            }
        }
        ui->comboBox_tcpwhoid->addItem("ALL");
        connect(m_tcp,&QTcpSocket::readyRead,this,[=](){   //接收数据
            QByteArray tcpdata = m_tcp->readAll();
            ui->textEdit_tcpacceptmsg->append("客户端：" + tcpdata);
        });

        connect(m_tcp,&QTcpSocket::disconnected,this,[=](){
            m_tcp->close();
            m_tcp->deleteLater();
            ui->textEdit_tcpacceptmsg->append("客户端已断开!");
            ui->comboBox_tcpwhoid->clear();
        });
    });
    //客户端
    client_tcp = new QTcpSocket(this);
    connect(client_tcp,&QTcpSocket::readyRead,this,[=](){  //接收数据
        QByteArray data = client_tcp->readAll();
        ui->textEdit_clientaccept->append("服务端："+ data);
    });
    connect(client_tcp,&QTcpSocket::connected,this,[=](){  //成功连接
        ui->textEdit_clientaccept->append("连接成功!");
        ui->pushButton_clientout->setEnabled(true);
        ui->pushButton_clientsend->setEnabled(true);
        ui->pushButton_clientconnect->setEnabled(false);
    });
    connect(client_tcp,&QTcpSocket::disconnected,this,[=](){  //断开
        on_pushButton_clientout_clicked();
        ui->textEdit_clientaccept->append("断开连接!");
    });
    ...
    ui->pushButton_clientout->setEnabled(false);
    ui->pushButton_clientsend->setEnabled(false);
    ui->pushButton_newnumber->setEnabled(false);
    ui->pushButton_tcpout->setEnabled(false);
    ui->pushButton_tcpsend->setEnabled(false);
    ...
    ui->pushButton_findred->setEnabled(false);
    ui->pushButton_clearcolor->setEnabled(false);
    ...
    ui->spinBox_colorbuild1->setRange(0,255);
    ui->spinBox_colorbuild1->setValue(200);
    ui->spinBox_colorbuild2->setRange(0,255);
    ui->spinBox_colorbuild2->setValue(255);
    ui->spinBox_colorareavalue1->setRange(0,500000);
    ui->spinBox_colorareavalue2->setRange(0,500000);
    ui->spinBox_colorareavalue1->setValue(0);
    ui->spinBox_colorareavalue2->setValue(500000);
...
{
    Path = QFileDialog::getOpenFileName(this,"文件夹：","/"); //通过文件夹打开
...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    QString sizeStr = QString("%1x%2").arg(size.width()).arg(size.height());  //窗口大小转 qstring
    ui->label_windowsize->setText(sizeStr);
}

void MainWindow::on_pushButton_0_video_clicked()
{
    camera_video = ui->comboBox_cameratype->currentText() + ui->comboBox_camera->currentText(); //获取摄像头名称
    qDebug() << camera_video;
    QMessageBox::information(this,"提示","已添加外设摄像头:"+camera_video);
    ui->pushButton_3->setEnabled(true);
}

void MainWindow::on_pushButton_findcamera_clicked()
{
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    ...
    if (ret == QMessageBox::Yes) {
        if (stop_msg == "no"){
            ...
            QMessageBox::information(this,"提示","tcp通信监听未关闭！");
            event->ignore();
        }else {
            event->accept();
        }
...
void MainWindow::on_pushButton_clear_clicked()
{
    ...
    ui->pushButton_gray->setEnabled(false);   //使控件失效
    ui->pushButton_save->setEnabled(false);
    ui->pushButton_clear->setEnabled(false);
    ui->pushButton_Threshold1->setEnabled(false);
    ui->pushButton_emphasize1->setEnabled(false);
...
{
    Rgb1ToGray(ho_image, &ho_image);    //转灰度图
    DispObj(ho_image, hv_WindowHandle);  //显示
    ui->pushButton_Threshold1->setEnabled(true);
...
{
    Threshold1 = value1;  //获取滑动条值
    Threshold_msg1 = QString("%1").arg(value1); //转字符串
    ui->label_Threshold1->setText(Threshold_msg1);  //在label中显示数值
...
void MainWindow::on_pushButton_colorfile_clicked()
{
    QString Pathfilecolor = QFileDialog::getOpenFileName(this,"文件夹：","/");
    ui->lineEdit_color->setText(Pathfilecolor);
    CloseWindow(hv_WindowHandle4);  //关闭当前窗口
    winId_color = (Hlong)this->ui->graphicsView_color->winId();  //绑定窗口
    ReadImage(&ho_Imagecolor, Pathfilecolor.toStdString().c_str());  //读取图片
    GetImageSize(ho_Imagecolor, &hv_Widthcolor, &hv_Heightcolor);  //获取图片尺寸
    OpenWindow(0,0,ui->graphicsView_color->width(),ui->graphicsView_color->height(),winId_color,"visible","",&hv_WindowHandle4); // 打开窗口
    SetWindowAttr("background_color","black"); // 设置背景颜色
    SetLineWidth(hv_WindowHandle4,3);
    SetPart(hv_WindowHandle4,0, 0, hv_Heightcolor - 1, hv_Widthcolor - 1); //设置图像显示为充满图像
    DispObj(ho_Imagecolor, hv_WindowHandle4); // 显示图片
    statusBar->showMessage(Pathfilecolor+":图片已载入窗口！");
    ui->pushButton_findred->setEnabled(true);
    ui->pushButton_clearcolor->setEnabled(true);
}

void MainWindow::on_pushButton_clearcolor_clicked()
{
    CloseWindow(hv_WindowHandle4);
    statusBar->showMessage("窗口已清理！");
    ui->pushButton_findred->setEnabled(false);
    ui->pushButton_clearcolor->setEnabled(false);
}

void MainWindow::on_pushButton_findred_clicked()
{
    QString colorstyle = ui->comboBox_colorchoice->currentText();
    if(colorstyle == "green"){
        colorbuild1 = 78;
        colorbuild2 = 97;
        ui->spinBox_colorbuild1->setValue(colorbuild1);
        ui->spinBox_colorbuild2->setValue(colorbuild2);
    }
    if(colorstyle == "blue"){
        colorbuild1 = 128;
        colorbuild2 = 147;
    ...
    if(colorstyle == "orange"){
        colorbuild1 = 10;
        colorbuild2 = 19;
        ui->spinBox_colorbuild1->setValue(colorbuild1);
        ui->spinBox_colorbuild2->setValue(colorbuild2);
    }
    MeanImage(ho_Imagecolor, &ho_ImageMeancolor, 3, 3);
    Decompose3(ho_ImageMeancolor, &ho_Redcolor, &ho_Greencolor, &ho_Bluecolor);  //将RGB图根据通道拆分成三张图
    TransFromRgb(ho_Redcolor, ho_Greencolor, ho_Bluecolor, &ho_Huecolor, &ho_Saturationcolor,  //将RGB转为HSV
                 &ho_Intensitycolor, "hsv");
    Threshold(ho_Saturationcolor, &ho_HighSaturationcolor, 60, 255);  //对饱和度阈值处理（限制饱和度范围）
    ReduceDomain(ho_Huecolor, ho_HighSaturationcolor, &ho_HueHighSaturationcolor);  //抠图
    Threshold(ho_HueHighSaturationcolor, &ho_greencolor, ui->spinBox_colorbuild1->value(), ui->spinBox_colorbuild2->value()); //对色度阈值处理（限制色度范围）
    Connection(ho_greencolor, &ho_ConnectedRegionscolor);     //计算连通域
    SelectShape(ho_ConnectedRegionscolor, &ho_SelectedRegionscolor, "area", "and",
                ui->spinBox_colorareavalue1->value(),ui->spinBox_colorareavalue2->value());  //选择形状（特征）
    SmallestRectangle2(ho_SelectedRegionscolor, &hv_Rowcolor, &hv_Columncolor, &hv_Phicolor,  //找最小外接矩形
                       &hv_Lengthcolor1, &hv_Lengthcolor2);
    SetDraw(hv_WindowHandle4,"margin");
    SetLineWidth(hv_WindowHandle4,2);
    SetColor(hv_WindowHandle4,"red");
    //画
    GenRectangle2(&ho_Rectanglecolor, hv_Rowcolor, hv_Columncolor, hv_Phicolor, hv_Lengthcolor1+2,
                  hv_Lengthcolor2+2);
    CountObj(ho_SelectedRegionscolor, &hv_Numbercolor);
    HTuple end_val30 = hv_Numbercolor;
    HTuple step_val30 = 1;
    HTuple hv_i, hv_row, hv_col;
    for (hv_i=1; hv_i.Continue(end_val30, step_val30); hv_i += step_val30)  //循环获取点
    {
        hv_row = HTuple(hv_Rowcolor[hv_i-1]);
        hv_col = HTuple(hv_Columncolor[hv_i-1]);
        ui->textEdit_color->append(QString::number((hv_i).D())+". ("+QString::number((hv_row).D())+", "
                                   +QString::number((hv_col).D())+")");
        qDebug() << QString::number((hv_row).D());
    }
    ClearWindow(hv_WindowHandle4);
    DispObj(ho_Imagecolor,hv_WindowHandle4);
    DispObj(ho_Rectanglecolor, hv_WindowHandle4);
}

void MainWindow::on_pushButton_7_clicked()
{
    ui->textEdit_color->clear();
}

void MainWindow::on_pushButton_tcpstart_clicked()
{
    unsigned short tcpcom = ui->lineEdit_tcpcom->text().toUShort();
    server->listen(QHostAddress(ui->comboBox_tcpip->currentText()),tcpcom);  //监听
    ui->textEdit_tcpacceptmsg->append("开始监听！");
    ui->pushButton_tcpout->setEnabled(true);
    ui->pushButton_tcpstart->setEnabled(false);
    tcpstart = "ok";
}

void MainWindow::on_pushButton_tcpout_clicked()
{
    ...
    server->close();
    ui->textEdit_tcpacceptmsg->append("结束监听!");
    ui->comboBox_tcpwhoid->clear();
    ui->pushButton_tcpout->setEnabled(false);
    ui->pushButton_tcpsend->setEnabled(false);
    ui->pushButton_newnumber->setEnabled(false);
...
{
    QList<QTcpSocket*> tcpSocketClients = server->findChildren<QTcpSocket*>();
    QString msg = ui->textEdit_tcpsendmsg->toPlainText();  //获取要发送的内容
    if(ui->comboBox_tcpwhoid->currentText() != "ALL"){
        QString currentName = ui->comboBox_tcpwhoid->currentText();
        for(QTcpSocket* tmp:tcpSocketClients){
            if(QString::number(tmp->peerPort()) == currentName){
                tmp->write(msg.toUtf8());
            }
        }
    }else{
        for(QTcpSocket* tmp:tcpSocketClients){
    ...
    ui->textEdit_tcpacceptmsg->append("服务端：" + msg);  //显示要发送的内容
}

void MainWindow::on_pushButton_newnumber_clicked()
{
    ui->comboBox_tcpwhoid->clear();
    QList<QTcpSocket*> tcpSocketClients = server->findChildren<QTcpSocket*>();  //获取客户端号
    for(QTcpSocket* tmp:tcpSocketClients){
...
void MainWindow::on_pushButton_clientconnect_clicked()
{
    client_tcp->connectToHost(ui->lineEdit_clientip->text(),ui->lineEdit_clientnumber->text().toInt());
    ui->textEdit_clientaccept->append("开始连接！");
}

void MainWindow::on_pushButton_clientsend_clicked()
{
    QString msg = ui->textEdit_clientmsg->toPlainText();
    client_tcp->write(msg.toUtf8());
    ui->textEdit_clientaccept->append("客户端：" + msg);
}

void MainWindow::on_pushButton_clientout_clicked()  //断开连接
{
    client_tcp->close();
    // client_tcp->deleteLater();
    ui->pushButton_clientout->setEnabled(false);
    ui->pushButton_clientsend->setEnabled(false);
    ui->pushButton_clientconnect->setEnabled(true);
}
